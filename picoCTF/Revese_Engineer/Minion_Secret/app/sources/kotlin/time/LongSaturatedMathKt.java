...
public final class LongSaturatedMathKt {
    ...
    public static final long m1583saturatingAddpTJri5U(long longNs, long duration) {
        long durationNs = Duration.m1475getInWholeNanosecondsimpl(duration);
        if (((longNs - 1) | 1) == Long.MAX_VALUE) {
            return m1582checkInfiniteSumDefinedPjuGub4(longNs, duration, durationNs);
        }
        if ((1 | (durationNs - 1)) == Long.MAX_VALUE) {
            return m1584saturatingAddInHalvespTJri5U(longNs, duration);
        }
    ...
    private static final long m1582checkInfiniteSumDefinedPjuGub4(long longNs, long duration, long durationNs) {
        if (!Duration.m1487isInfiniteimpl(duration) || (longNs ^ durationNs) >= 0) {
            return longNs;
        }
    ...
    private static final long m1584saturatingAddInHalvespTJri5U(long longNs, long duration) {
        long half = Duration.m1458divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1475getInWholeNanosecondsimpl(half);
        if ((1 | ($this$isSaturated$iv - 1)) == Long.MAX_VALUE) {
            return (long) (longNs + Duration.m1498toDoubleimpl(duration, DurationUnit.NANOSECONDS));
        }
        return m1583saturatingAddpTJri5U(m1583saturatingAddpTJri5U(longNs, half), Duration.m1490minusLRDsOJo(duration, half));
    }

    public static final long saturatingDiff(long valueNs, long originNs) {
        if ((1 | (originNs - 1)) == Long.MAX_VALUE) {
            return Duration.m1507unaryMinusUwyO8pc(DurationKt.toDuration(originNs, DurationUnit.DAYS));
        }
    ...
    public static final long saturatingOriginsDiff(long origin1Ns, long origin2Ns) {
        if (((origin2Ns - 1) | 1) == Long.MAX_VALUE) {
            return origin1Ns == origin2Ns ? Duration.INSTANCE.m1557getZEROUwyO8pc() : Duration.m1507unaryMinusUwyO8pc(DurationKt.toDuration(origin2Ns, DurationUnit.DAYS));
        }
    ...
    private static final long saturatingFiniteDiff(long value1Ns, long value2Ns) {
        ...
        if (((result ^ value1Ns) & (~(result ^ value2Ns))) < 0) {
            ...
            return Duration.m1491plusLRDsOJo(duration, DurationKt.toDuration(resultNs, DurationUnit.NANOSECONDS));
        }
