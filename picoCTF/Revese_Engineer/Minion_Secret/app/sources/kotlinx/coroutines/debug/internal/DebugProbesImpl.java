...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        if (objNewInstance != null) {
            objM129constructorimpl = Result.m129constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
            if (Result.m135isFailureimpl(objM129constructorimpl)) {
                objM129constructorimpl = null;
            }
            return (Function1) objM129constructorimpl;
        }
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            Result.Companion companion = Result.INSTANCE;
            DebugProbesImpl debugProbesImpl = this;
            objM129constructorimpl = Result.m129constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            objM129constructorimpl = Result.m129constructorimpl(ResultKt.createFailure(th));
        }
        ...
        StackTraceElement[] actualTrace = (StackTraceElement[]) objM129constructorimpl;
        if (actualTrace == null) {
            return coroutineTrace;
        }
        int index$iv = 0;
