...
final class ResumeAwaitOnCompletion<T> extends JobNode {
    ...
    public void invoke2(Throwable cause) {
        ...
        if (state instanceof CompletedExceptionally) {
            ...
            cancellableContinuationImpl.resumeWith(Result.m129constructorimpl(ResultKt.createFailure(((CompletedExceptionally) state).cause)));
        } else {
            CancellableContinuationImpl<T> cancellableContinuationImpl2 = this.continuation;
            Result.Companion companion2 = Result.INSTANCE;
            cancellableContinuationImpl2.resumeWith(Result.m129constructorimpl(JobSupportKt.unboxState(state)));
        }
    }
}
