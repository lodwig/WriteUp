...
public final class DatesKt {
    ...
    public static final Date toHttpDateOrNull(String toHttpDateOrNull) {
        ...
        Date date = STANDARD_DATE_FORMAT.get().parse(toHttpDateOrNull, position);
        if (position.getIndex() == toHttpDateOrNull.length()) {
            return date;
        }
        synchronized (BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                position.setIndex(0);
                Date date2 = format.parse(toHttpDateOrNull, position);
